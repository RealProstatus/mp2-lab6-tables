...
public:
	...
	void deleteRecord(TKey key) override {
		...
		Del_Rec(this->pRoot, key);
	}

    ~AVLTreeTable() {
        this->DeleteRecord(this->pRoot);
    }
...
protected:
	...
	int insBalTree(TreeNode<TKey, TValue>*& pNode, Record<TKey, TValue> rec) {
		...
		if (pNode == nullptr) {
			pNode = new TreeNode<TKey, TValue>{ rec, nullptr,nullptr, BALANCE::BOK };
			res = HEIGHT::INC;
			this->DataCount++;
		}
		else {
    ...
    int BalTreeLeft(TreeNode<TKey, TValue>*& pNode, MODE mode) {
        ...
        if (mode == MODE::INSERT) {
            ...
            if (pNode->balance == BALANCE::RIGHT) {
                pNode->balance = BALANCE::BOK;
                res = HEIGHT::OK;
            }
            else if (pNode->balance == BALANCE::BOK) {
                ...
                if (p1->balance == BALANCE::LEFT) {
                    ...
                    pNode = p1;
                    pNode->balance = BALANCE::BOK;
                    pNode->pRight->balance = BALANCE::BOK;
                    res = HEIGHT::OK;
                }
                else {
                    // LR-поворот
        ...
        if (mode == MODE::INSERT) {
            ...
            if (pNode->balance == BALANCE::LEFT) {
                ...
                if (p1->balance == BALANCE::RIGHT) {
                    ...
                    pNode = p1;
                    pNode->balance = BALANCE::BOK;
                    pNode->pLeft->balance = BALANCE::BOK;
                    res = HEIGHT::OK;
                }
                else {
                    // RL-поворот
